...
class Data2Tag(object):
    def __init__(self, config_df=None, feature_table=None, partition=None):
        ...
        if feature_table is None:
            self.feature_table = "chenyw_yzx.feature605_data2kw"
        else:
            self.feature_table = feature_table
        if partition is None:
            curr_month = datetime.datetime.today().strftime("%Y%m")
            if curr_month[4:] == '01':
                self.partition = str(int(curr_month[:4]) - 1) + '12'
            else:
                self.partition = str(int(curr_month) - 1)
        else:
            self.partition = partition
        self.day = self.partition + '01'
        self.month = self.partition

    def get_sql(self, feature, type=0):
        """
        :param feature:
        :param type: 1-black_list
                     2-continuous_var
                     3-class_var
        :return:
        """
        print("Now is dealing the var={}".format(feature))
        sql = ""
        if type == 1:
            sql = """select '{}' as feature, (case when {}='' then 'blank' else '1' end) as tag, count(1) as cnt from {} where month='{}' group by (case when {}='' then 'blank' else '1' end)""".format(
                feature, feature, self.feature_table, self.partition, feature)
        elif type == 2:
            feature_config = self.config_df[self.config_df["feature"] == feature]
            feature_config.index = ([i for i in range(feature_config.shape[0])])
            sql = sql + "select '{}' as feature,".format(feature)
            case_sql = "(case when {}='' then 'blank'".format(feature)
            for index, row in feature_config.iterrows():
                if index == 0:
                    case_sql = case_sql + " when float({})<={} then '{}'".format(
                        row['feature'], row['max_value'], row['tag'])
                elif index == len(feature_config) - 1:
                    case_sql = case_sql + " when float({})>{} then '{}'".format(
                        row['feature'], row['min_value'], row['tag'])
                else:
                    case_sql = case_sql + " when float({})<={} and float({})>{} then '{}'".format(
                        row['feature'], row['max_value'], row['feature'], row['min_value'], row['tag'])
            case_sql = case_sql + " else null end)"
            sql = sql + case_sql + " as tag, count(1) as cnt from {} where month='{}' group by ".format(self.feature_table, self.partition) + case_sql
        elif type == 3:
            feature_config = self.config_df[self.config_df["feature"] == feature]
            feature_config.index = ([i for i in range(feature_config.shape[0])])
            sql = sql + "select '{}' as feature,".format(feature)
            case_sql = "(case when {}='' then 'blank'".format(feature)
            for index, row in feature_config.iterrows():
                case_sql = case_sql + " when {}='{}' then '{}'".format(row['feature'], row['tag'], row['tag'])
            case_sql = case_sql + " else null end)"
            sql = sql + case_sql + " as tag, count(1) as cnt from {} where month='{}' group by '{}', ".format(
                self.feature_table, self.partition, feature) + case_sql

        sql = "insert into table chenyw_yzx.autolabel_map partition(month='{}',day='{}') ".format(
            self.month, self.day) + sql
        return sql

    def insert_into_hive(self, sql):
        cmd = """beeline -u jdbc:hive2://beeline-ds.bi.getui.com:10000 -n {} -w {} -e "{}" """.format(self.user_name,
                                                                                                      self.password_path,
                                                                                                      sql)
        os.system(cmd)
    ...
    def main(self):
        ...
        for c in self.blacklist_var:
            sql = self.get_sql(feature=c, type=1)
            self.insert_into_hive(sql)
        # 多线程insert
        p = Pool(4)
        for c in self.continuous_var:
            sql = self.get_sql(feature=c, type=2)
            p.apply_async(self.insert_into_hive, args=(sql))
        for c in self.class_var:
            sql = self.get_sql(feature=c, type=3)
            p.apply_async(self.insert_into_hive, args=(sql))
        p.close()
        p.join()
