...
def get_better_quct_bin(Y, X, n=5):
    min_value = np.min(X)
    max_value = np.max(X)
    ll = [i for i in range(max_value-min_value+1)]
    cut_points = []
    groups = 1
    while groups <= n:
        cut_points.append(np.percentile(ll, int(groups/n*100) + min_value))
        groups = groups + 1
    df = pd.DataFrame({"X": X, "Y": Y})
    df["level"] = df.X.map(lambda x: [x <= i for i in cut_points].index(True))