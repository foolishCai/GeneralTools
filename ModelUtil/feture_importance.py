...
class Exploratory_Data_Analysis(object):

    def __init__(self,data,continuous_col,category_col,target,drop_col):
        '''
        param data:数据集【dataframe】
        param continuous_col:连续型变量列表集合【list】
        param category_col:类别型变量列表集合【list】
        param target:目标变量字段名【str】
        param drop_col:无效无用的变量列表集合【list】
        '''
        self.data=data
    ...
    def Lgb_ImportantFeature(self, sample_num, figsize, max_num_features=20):
        ...
        # 画图
        lgb.plot_importance(gbm, importance_type='gain', max_num_features=max_num_features, precision=0,
                            figsize=figsize)
