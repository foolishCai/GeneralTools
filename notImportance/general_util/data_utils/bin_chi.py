...
note: 卡方分箱, class可以处理一整个dataframe
'''
...
class ChiBin(object):
    ...
    def __init__(self, df, feature, target):
        ...
        :param feature: 这个分箱的列，数据结构是一个字段数组，数组元素为字典
         例如colList=[
              {'col':'age','bandNum':6},

         ]
         'col':列名
         'bandNum':分箱数，如果该值缺失，则使用卡方阈值的方式进行分箱
        ...
        for item in feature:
            col_dict = {}
            col_dict['col'] = item['col']
            col_dict['bandCol'] = item['col']+'_bin'
            if 'bandNum' in item.keys():
                col_dict['bandNum'] = item['bandNum']
            self.colList.append(col_dict)
        self.target = target
    ...
    def to_band(self):
        for i in range(len(self.colList)):
            ...
            if 'bandNum' in colParam.keys():
                cutOffPoints = ChiMerge_MaxInterval_Original(self.df, colParam['col'], self.target, colParam['bandNum'])
            else:
                cutOffPoints = ChiMerge_MinChisq(self.df, colParam['col'], self.target)
                colParam['bandNum'] = len(cutOffPoints)+1
            print(cutOffPoints)
            ...
            self.df[colParam['bandCol']] = self.df[colParam['bandCol']].astype(int)
            # 到此分箱结束，下面判断单调性
